...
- Cost model: **commission-rate based** — `cost = commission_rate × expected_gmv`. Commission rates are drawn from Uniform(0.08, 0.25) for Nano/Micro and Uniform(0.10, 0.20) for Macro/Mega, calibrated to TikTok Shop SEA affiliate commission ranges. Because cost scales with expected GMV, high-follower KOLs are both more valuable and more expensive, preserving the local-optimum trap structure.
- Country + category: uniform over {MY, ID, TH, PH, SG, VN} × {beauty, fashion, home, fmcg}

The generator accepts a `--seed` parameter for reproducibility. The commission-rate cost model preserves the classic trap: a Macro/Mega KOL may consume 60–80% of the budget while a portfolio of Micro+Nano KOLs at the same total cost produces higher aggregate GMV through higher engagement multipliers.
...
*(commission-rate cost model; budget = N × $25; seed=42)*

| N | Budget | GA | TS | SA | GR | HC | RS |
|---|--------|----|----|----|----|----|----|
| 20 | $500 | **1,834** | **1,834** | **1,834** | 1,746 | 1,044 | **1,834** |
| 50 | $1,250 | **10,767** | 10,603 | **10,767** | 10,603 | 4,323 | 10,415 |
| 100 | $2,500 | 19,759 | **19,988** | 19,437 | **19,988** | 10,024 | 17,729 |
| 150 | $3,750 | 62,181 | — | 31,134 | **62,221** | 18,369 | 57,239 |
| 200 | $5,000 | **78,356** | — | 43,635 | **78,356** | 22,512 | 67,949 |

*TS not benchmarked at N>100 due to O(N²) neighbourhood enumeration.*
*GMV values are lower than prior table because commission-rate costs now scale with GMV (higher-GMV KOLs cost proportionally more), reducing the number of KOLs that fit within budget.*
