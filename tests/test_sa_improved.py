"""
Unit tests for the improved Simulated Annealing algorithm.
"""
import sys, os
sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

import json
import pytest
from engine.models import KOL
from engine.optimization.sa_improved import simulated_annealing_improved
from engine.optimization.hill_climber import hill_climber
from engine.fitness import summarize_state


@pytest.fixture(scope="module")
def kols_my_beauty():
    """Load and filter MY beauty KOLs once per test module."""
    with open("data/sample_kols.json") as f:
        kols_data = json.load(f)
    kols = [KOL(**item) for item in kols_data]
    return [k for k in kols if k.country == "MY" and k.category == "beauty"]


BUDGET_LIMIT = 5000


def test_sai_state_format(kols_my_beauty):
    """Test that simulated_annealing_improved returns a valid binary state."""
    state, _, _ = simulated_annealing_improved(
        kols_my_beauty, budget=BUDGET_LIMIT, seed=42
    )
    assert len(state) == len(kols_my_beauty)
    assert all(selected in (0, 1) for selected in state)


def test_sai_budget_compliance(kols_my_beauty):
    """Test that the solution does not exceed the budget limit."""
    state, _, _ = simulated_annealing_improved(
        kols_my_beauty, budget=BUDGET_LIMIT, seed=42
    )
    total_cost = sum(
        kols_my_beauty[i].cost for i, selected in enumerate(state) if selected
    )
    assert total_cost <= BUDGET_LIMIT, f"Cost {total_cost} exceeds budget {BUDGET_LIMIT}"


def test_sai_outperforms_hill_climber(kols_my_beauty):
    """Test that SAI's average GMV over multiple seeds is at least as good as HC."""
    seeds = [42, 1, 2, 3, 4, 5, 6, 7, 8, 9]
    sai_gmvs = []
    hc_gmvs = []

    for seed in seeds:
        sai_state, _, _ = simulated_annealing_improved(
            kols_my_beauty, budget=BUDGET_LIMIT, seed=seed
        )
        hc_state, _, _ = hill_climber(
            kols_my_beauty, budget=BUDGET_LIMIT, seed=seed
        )
        sai_gmvs.append(summarize_state(sai_state, kols_my_beauty)["total_gmv"])
        hc_gmvs.append(summarize_state(hc_state, kols_my_beauty)["total_gmv"])

    avg_sai = sum(sai_gmvs) / len(sai_gmvs)
    avg_hc = sum(hc_gmvs) / len(hc_gmvs)

    assert avg_sai >= avg_hc, (
        f"SAI average GMV ({avg_sai:.2f}) is worse than HC ({avg_hc:.2f})"
    )
